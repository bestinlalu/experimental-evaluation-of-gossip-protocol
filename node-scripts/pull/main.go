...
type NodeState struct {
	...
	Timestamp  int64  `json:"timestamp"` // Unix epoch in nanoseconds
}
...
func (n *Node) handlePullRequest(replyToAddress string) {
	...
	resp := GossipMessage{Type: "PULL_RESP", State: n.StateMap}
		...
		if isUpdated && n.kafkaWriter != nil {
			...
			newKafkaEvents = append(newKafkaEvents, KafkaEvent{
				...
				GossipDigest: GossipDigest{
					...
					Timestamp:  ts,
					IsAlive:    true, // Since we just got a valid update, we assume it's alive
				},
			})
		}
	}
...
func (n *Node) StartGossiping(interval time.Duration) {
	...
	for {
		...
		req := GossipMessage{Type: "PULL_REQ", ReplyTo: n.Address}
