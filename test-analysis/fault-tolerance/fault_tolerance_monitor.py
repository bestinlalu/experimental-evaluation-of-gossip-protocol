...
#         alive    = [f"ALIVE -> {p}" for p in peers if p in active_nodes]
#         dead     = [f"DEAD -> {p}" for p in peers if p not in active_nodes]
...
def print_neighbors(neighbor_map, active_nodes, label=""):
    ...
    for node in sorted(neighbor_map.keys()):
        peers   = neighbor_map[node]
        status  = "ALIVE" if node in active_nodes else "DEAD"
        print(f"  {status} {node}")
        for p in peers:
            p_status = "Alive" if p in active_nodes else "Dead"
            print(f"       └── {p}  [{p_status}]")
        if not peers:
            print(f"       └── (no peers)")
    print(f"  {'─'*55}")
...
def run():
    ...
    try:
        ...
        print(f"  Only counting UIDs with first GossipRecord ID > {anchor}\n")
        p_done = set()
        ...
        if b_dur and p_dur:
            ...
            print(f"  {'Convergence delta':<30} | {delta:+.4f}s")
